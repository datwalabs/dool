    ...
    "tasks": [
        {
            ...
            "operator_id": 1,
            "sequence": 1,
            "task_param": "WWFWEFEWDWE",
            "operator": {
                "operator_id": 1,
                "operator_name": "Python Operator",
                "operator_slug": "python_operator",
...
export default function JobIntegration() {
  ...
  // Convert Jobresponse.tasks to stepList format by grouping based on sequence
  useEffect(() => {
    if (response && response.tasks) {
      const groupedTasks = response.tasks.reduce((acc: jobIntegration[][], task: any) => {
        const newTask = {
          operator_name: task.operator.operator_name,
          operator_id: task.operator.operator_id,
          description: task.operator.description,
          taskName: task.task_name,
          taskParams: task.task_param,
        };

        // Find or create a group based on the task's sequence
        if (!acc[task.sequence - 1]) {
          acc[task.sequence - 1] = []; // Create a new group if it doesn't exist
        }

        acc[task.sequence - 1].push(newTask); // Add task to the correct group

        return acc;
      }, []);

      setSteplist(groupedTasks); // Set the stepList with grouped tasks by sequence
    }
  }, [Listjob]);
